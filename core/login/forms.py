    ...
    def clean(self):
        ...
        if queryset.exists():
            ...
            if authenticate(username=username, password=password) is None:
                ...
                if intent > 4:  # Cambiar a 4 intentos fallidos permitidos
                    user.is_active = False
                    user.save()
                    raise forms.ValidationError('El usuario ha sido bloqueado por superar el límite de intentos fallidos en un día.')
                count = 5 - intent  # Cambiar a 5 intentos permitidos
                raise forms.ValidationError(f"La contraseña ingresada es incorrecta, por favor intentelo de nuevo. Le quedan {count} {'intento' if count == 1 else 'intentos'}. Si supera los 5 intentos fallidos su cuenta sera bloqueada.")
            UserAccess(user=user).save()
            return cleaned
        raise forms.ValidationError('Por favor introduzca el usuario y la contraseña correctos. Ambos campos pueden ser sensibles a mayúsculas.')
